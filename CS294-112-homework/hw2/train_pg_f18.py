...
class Agent(object):
    ...
    def sample_action(self, policy_parameters):
        ...
        if self.discrete:
            ...
            z = tf.random_normal(tf.shape(sy_mean), 0, 1)
    ...
    def build_computation_graph(self):
        ...
        # multiply by negative one for gradient ascent
    ...
    def sample_trajectories(self, itr, env):
        ...
        paths = []
        while True:
            animate_this_episode=(len(paths)==0 and (itr % 80 == 0) and (itr != 0) and self.animate)
    ...
    def sum_of_rewards(self, re_n):
        ...
        for path in re_n:
            # DP solution for rewards to go using Horner's rule
            q_pn = []
            q = 0
            # reverse list 
            for r in path[::-1]:
    ...
    def compute_advantage(self, ob_no, q_n):
        ...
        if self.nn_baseline:
            ...
            bl_pred = self.sess.run(self.baseline_prediction, feed_dict={
                self.sy_ob_no : ob_no
            })
            # normalize prediced return to go for actions rel to actual return to go values
            b_n = (bl_pred - np.mean(bl_pred)) / (np.var(bl_pred) + 1e-6)
            b_n = np.mean(q_n) + b_n * np.var(q_n)
            adv_n = q_n - b_n
        else:
            adv_n = q_n.copy()
        return adv_n
    ...
    def estimate_return(self, ob_no, re_n):
        ...
        if self.normalize_advantages:
            ...
            adv_n = (adv_n - np.mean(adv_n)) / (np.var(adv_n) + 1e-6)
        return q_n, adv_n
    ...
    def update_parameters(self, ob_no, ac_na, q_n, adv_n):
        ...
        if self.nn_baseline:
            ...
            # YOUR_CODE_HERE
            q_nn = (np.mean(q_n) - q_n) / (np.var(q_n) + 1e-6)
            self.sess.run(self.baseline_update_op, feed_dict={
                self.sy_ob_no: ob_no,
                self.sy_target_n: q_nn,
            })
