...
class SAC:
    ...
    def _policy_loss_for(self, policy, q_function, q_function2, value_function):
        ...
        if q_function2 is not None:
            q_pred = tf.minimum(q_function((self._observations_ph, actions)),
                                q_function2((self._observations_ph, actions)))
        else:
            q_pred = q_function((self._observations_ph, actions))
        
        if not self._reparameterize:
            ### Problem 1.3.A
            baseline = value_function(self._observations_ph)
            policy_loss_bs = log_pis * (tf.stop_gradient(self._alpha * log_pis - q_pred) + baseline)
            policy_loss = tf.reduce_mean(policy_loss_bs)
        else:
            ### Problem 1.3.B
            policy_loss = tf.reduce_mean(self._alpha * log_pis - q_pred)

        return policy_loss

    def _value_function_loss_for(self, policy, q_function, q_function2, value_function):
        ### Problem 1.2.A
        v_pred = value_function(self._observations_ph)
        actions, log_pis = policy(self._observations_ph)
        ...
        v_target = q_pred - self._alpha * log_pis    

        return tf.losses.mean_squared_error(v_target, v_pred)


    def _q_function_loss_for(self, q_function, target_value_function):
        ### Problem 1.1.A
        q_pred = q_function((self._observations_ph, self._actions_ph))
        target_v_pred = target_value_function(self._next_observations_ph)
        target_pred_ph = self._rewards_ph + (1 - self._terminals_ph) * self._discount * target_v_pred

        return tf.losses.mean_squared_error(target_pred_ph, q_pred)
