...
class ObservedPointEnv(Env):
    ...
    def set_task_enc(self):
        # hot encode task
        self.task_enc = np.zeros((len(self.tasks)))
        self.task_enc[self.task_idx] = 1

    def enc_state(self, state):
        return np.concatenate((self._state, self.task_enc))
    ...
    def _get_obs(self):
        # aug obs with task encoding
        self._state = self.enc_state(self._state)
    ...
    def step(self, action):
        ...
        # move to next state
        self._state = self._state[:2] + action
