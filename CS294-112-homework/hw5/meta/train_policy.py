...
def build_rnn(x, h, output_size, scope, n_layers, size, activation=tf.tanh, output_activation=None, regularizer=None):
    ...
    x = build_mlp(x, size, scope, n_layers + 1, size, activation=activation, output_activation=activation, regularizer=regularizer)
    x, h = tf.nn.dynamic_rnn(gru_cell, x, initial_state=h)
    x = tf.reshape(x[:, -1, :], [-1, h.shape[1]])
    ...
    def sample_trajectory(self, env, animate_this_episode, is_evaluation):
        ...
        while True:
            ...
            if ep_steps == 0:
                ...
                # YOUR CODE HERE
                ac, rew, done = np.zeros(self.ac_dim), np.zeros(self.reward_dim), np.zeros(self.terminal_dim)
            ...
            # ensure len is self.history
            if steps >= self.history:
                in_ = meta_obs[steps - self.history: steps]
            else:
                zeros = np.zeros((self.history - steps, self.meta_ob_dim))
                in_ = np.concatenate((zeros, meta_obs[:steps]), axis=0)
