...
class Agent(object):
    ...
    def estimate_advantage(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        critic_pred_next_ob = self.sess.run(self.critic_prediction, feed_dict={self.sy_ob_no: next_ob_no})
        q_n = re_n + (1 - terminal_n) * self.gamma * critic_pred_next_ob

        critic_pred_ob = self.sess.run(self.critic_prediction, feed_dict={self.sy_ob_no: ob_no})
        adv_n = q_n - critic_pred_ob
    ...
    def update_critic(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        for _ in range(self.num_target_updates):
            ...
            # compute y = r(s,a) + gamma * V(s')
            v_next_ob_no = self.sess.run(self.critic_prediction, feed_dict={
                self.sy_ob_no: next_ob_no
            })
            target_n = re_n + (1 - terminal_n) * self.gamma * v_next_ob_no

            for _ in range(self.num_grad_steps_per_target_update):
                # predict V(s) and update on loss V(s) - y
